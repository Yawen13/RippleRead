...
def update_progress(item_id: int, body, user_id: int = 1):
    ...
    if body.progress >= 0:
        ...
        if new_progress >= 100 and old_progress < 100:
            from services.notification_service import check_and_notify_book_finished
            check_and_notify_book_finished(user_id, item_id, book_title)

        from services.notification_service import check_and_notify_daily_goal, check_and_notify_streak_milestone
